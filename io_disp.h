...
public:
      ...
      for (int32_t u=0; u<width_; u++) {
        ...
        if (isValid(u,v)) {
          ...
          if (count>=1) {
            ...
            if (u1>0 && u2<width_-1) {
              float d_ipol = min(getDisp(u1-1,v),getDisp(u2+1,v));
  ...
  void readDisparityMap (png::image<png::gray_pixel_16> image) {
  ...
  void writeFalseColors (const std::string file_name, float max_val) {
    ...
    for (int32_t v=0; v<height_; v++) {
      for (int32_t u=0; u<width_; u++) {
        ...
        float val = min(std::max(getDisp(u,v)/max_val,0.0f),1.0f);
